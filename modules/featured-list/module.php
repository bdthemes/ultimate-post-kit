...
class Module extends Ultimate_Post_Kit_Module_Base {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_nopriv_upk_featured_list_loadmore_posts', [ $this, 'callback_ajax_loadmore_posts' ] );
		add_action( 'wp_ajax_upk_featured_list_loadmore_posts', [ $this, 'callback_ajax_loadmore_posts' ] );
	}
				...
				<div <?php if ( ! empty( $settings['global_link'] ) && $settings['global_link'] === 'yes' ) { printf( 'onclick="window.open(\'%s\', \'_self\')"', esc_url( $post_link ) ); } ?> class="upk-item">
					<div class="upk-item-box">
						...
						<div class="upk-content">
							<?php if ( $settings['show_counter_number'] === 'yes' && $settings['layout_style'] === 'style-1' ) : ?>
									...
									<div class="upk-category">
										...
										if ( $categories ) {
											foreach ( $categories as $category ) {
												...
												echo '<a href="' . esc_url( get_category_link( $category->term_id ) ) . '"><span style="background-color:' . esc_attr( $bg_color ) . '"></span>' . esc_html( $category->name ) . '</a>';
											}
										}
										?>
									</div>
