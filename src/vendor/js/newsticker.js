	...
	$.upkNewsTicker = function (element, options) {
		...
		var setContainerWidth = function () {
			if (ticker._label.length > 0) {
				if (ticker.settings.direction == 'rtl') ticker._news.css({ right: ticker._label.outerWidth() });
				else ticker._news.css({ left: ticker._label.outerWidth() });
			}

			if (ticker._controls.length > 0) {
				var controlsWidth = ticker._controls.outerWidth();
				if (ticker.settings.direction == 'rtl') ticker._news.css({ left: controlsWidth });
				else ticker._news.css({ right: controlsWidth });
			}
		...
		var startScrollAnimationRTL = function () {
			...
			if (ticker._pause === false) ticker._frameId = requestAnimationFrame(startScrollAnimationRTL);
		...
		var scrollPlaying = function () {
			if (ticker.settings.direction === 'rtl') {
				if (ticker._ul.width() > ticker._news.width()) startScrollAnimationRTL();
				else ticker._ul.css({ marginRight: 0 });
			} else if (ticker._ul.width() > ticker._news.width()) startScrollAnimationLTR();
			else ticker._ul.css({ marginLeft: 0 });
		};
		...
		var nextHandler = function () {
			switch (ticker.settings.effect) {
				...
				default:
					ticker._activeNews++;
					if (ticker._activeNews >= ticker._totalNews) ticker._activeNews = 0;
		...
		var prevHandler = function () {
			switch (ticker.settings.effect) {
				...
				default:
					ticker._activeNews--;
					if (ticker._activeNews < 0) ticker._activeNews = ticker._totalNews - 1;
		...
		var resizeEvent = function () {
			if (ticker._element.width() < 480) {
				ticker._label.hide();
				if (ticker.settings.direction == 'rtl') ticker._news.css({ right: 0 });
				else ticker._news.css({ left: 0 });
			} else {
				ticker._label.show();
				if (ticker.settings.direction == 'rtl') ticker._news.css({ right: ticker._label.outerWidth() });
				else ticker._news.css({ left: ticker._label.outerWidth() });
		...
		ticker.init = function () {
			...
			else ticker._action.find('span').removeClass('upk-news-ticker-play').addClass('upk-news-ticker-pause');
			...
			ticker._element.on('mouseleave', function (e) {
				...
				if (ticker.settings.pauseOnHover === true) {
					if (ticker.settings.autoPlay === true) ticker.play();
				} else {
					if (ticker.settings.autoPlay === true && ticker._pause === true) ticker.play();
				}
			});

			ticker._element.on('mouseenter', function () {
				if (ticker.settings.pauseOnHover === true) ticker.pause();
			});
