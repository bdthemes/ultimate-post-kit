(function ($, elementor) {
    ...
    async function initSwiper() {
      ...
      $slider.find(".upk-pholox-video-trigger").on("click", function () {
        ...
        var videoURL = $(this).data("src").split("?")[0],
          sliderWrapper = $slider.find(
            ".upk-main-slider .swiper-slide-active .upk-img-wrap",
          );
        ...
        sliderWrapper
          .find(".upk-video-iframe")
          .attr(
            "src",
            videoURL +
              "?autoplay=1&modestbranding=1&showinfo=0&rel=0&controls=0&loop=1",
          );
  ...
  jQuery(window).on("elementor/frontend/init", function () {
    elementorFrontend.hooks.addAction(
      "frontend/element_ready/upk-pholox-slider.default",
      widgetPholoxSlider,
    );
  });
