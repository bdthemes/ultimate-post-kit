(function ($) {
	...
	var UltimatePostKitEditor = {
		init: function () {
			elementor.channels.editor.on(
				'section:activated',
				UltimatePostKitEditor.onAnimatedBoxSectionActivated,
			);

			window.elementor.on('preview:loaded', function () {
				elementor.$preview[0].contentWindow.UltimatePostKitEditor =
					UltimatePostKitEditor;
		...
		savedBuilder: function () {
			if (UltimatePostKitEditor.shouldReload) {
				UltimatePostKitEditor.shouldReload = false;
		...
		onPreviewLoaded: function () {
			...
			elementorFrontend.hooks.addAction(
				...
				function ($scope) {
					$scope
						.find('.upk-elementor-template-edit-link')
	...
	elementor.hooks.addFilter('panel/elements/regionViews', function (panel) {
		...
		if (
			UltimatePostKitConfigEditor.pro_license_activated ||
			UltimatePostKitConfigEditor.promotional_widgets <= 0
		)
			return panel;
