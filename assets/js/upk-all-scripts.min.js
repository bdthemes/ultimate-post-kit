...
!function(i,n){"use strict";const e=function(i,n){let e,t=i.find(".upk-ajax-grid"),o=t.data("loadmore"),a=i.data("settings");if(void 0!==a&&(e=void 0!==a.upk_in_animation_show?"yes":""),!t.length)return;if("yes"!==o.loadmore_enable)return;const r=t.data("settings"),d=i.find(".upk-loadmore-btn"),l=i.find(".upk-loadmore-container"),s=t.find(".upk-ajax-grid-wrap"),u=s.data("in-animation-delay")?s.data("in-animation-delay"):200;let p=!1,c=!1,g=r.posts_per_page,f=s.find(".upk-item").length+1;const k=function(){if(c)return;var i,a=[];let k;const m=t?.attr("class")?.match(/upk-([a-z0-9_-]+)-(grid|list)/);if(m){k=`upk_${m[1].replace(/-/g,"_")}_${m[2]}_loadmore_posts`}let _={action:k,settings:r,per_page:r.ajax_item_load,offset:g,animation:e,slide_index:f};jQuery.ajax({url:window.UltimatePostKitConfig.ajaxurl,type:"post",data:_,success:function(t){if(n(s).append(t.markup),g+=r.ajax_item_load,f+=r?.ajax_item_load,p=!1,"yes"===o.loadmore_btn&&d.html("Load More"),n(t.markup).length<r.ajax_item_load&&(c=!0,d.hide(),l.hide()),"yes"===e){function k(){i||(i=window.setInterval((function(){a.length?(n(a.shift()).addClass("is-inview"),k()):(window.clearInterval(i),i=null)}),u))}!function(i,n){var e=arguments.length>2&&void 0!==arguments[2]?arguments[2]:{};e.rootMargin=e.rootMargin||"10% 0px 0px 0px",new IntersectionObserver((function(i,t){i.forEach((function(i){i.isIntersecting&&(n(i),e.loop||t.unobserve(i.target))}))}),e).observe(i)}(n(".upk-ajax-grid .upk-item:not(.is-inview)")[0],(function(){a.push(n(".upk-ajax-grid .upk-item:not(.is-inview)")),k()}),{root:null,rootMargin:"0px",threshold:.8})}}})};"yes"===o.loadmore_btn&&n(d).on("click",(function(){p?p=!1:(p=!0,d.html("loading..."),k())})),"yes"===o.infinite_scroll&&n(window).on("scroll",(function(){n(window).scrollTop()==n(document).height()-n(window).height()&&(n(".upk-ajax-loading").css("display","block"),k())}))};jQuery(window).on("elementor/frontend/init",(function(){["upk-amox-grid","upk-kalon-grid","upk-alex-grid","upk-alice-grid","upk-alter-grid","upk-elite-grid","upk-hazel-grid","upk-gratis-grid","upk-maple-grid","upk-pixina-grid","upk-ramble-grid","upk-wixer-grid","upk-tiny-list"].forEach((i=>{elementorFrontend.hooks.addAction(`frontend/element_ready/${i}.default`,e)}))}))}(jQuery,window.elementorFrontend);
