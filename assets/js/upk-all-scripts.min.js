...
!function(i,n){"use strict";const t=function(i,n){let t,e=i.find(".upk-ajax-grid"),o=e.data("loadmore"),a=i.data("settings");if(void 0!==a&&(t=void 0!==a.upk_in_animation_show?"yes":""),!e.length)return;if("yes"!==o.loadmore_enable)return;const r=e.data("settings"),d=i.find(".upk-loadmore-btn"),l=i.find(".upk-loadmore-container"),s=e.find(".upk-ajax-grid-wrap"),u=s.data("in-animation-delay")?s.data("in-animation-delay"):200;let p=!1,c=!1,g=r.posts_per_page,k=s.find(".upk-item").length+1;const f=function(){if(c)return;var i,a=[];let f;const m=e?.attr("class")?.match(/upk-([a-z0-9_-]+)-(grid|list)/);if(m){f=`upk_${m[1].replace(/-/g,"_")}_${m[2]}_loadmore_posts`}let _={action:f,settings:r,per_page:r.ajax_item_load,offset:g,animation:t,slide_index:k};jQuery.ajax({url:window.UltimatePostKitConfig.ajaxurl,type:"post",data:_,success:function(e){if(n(s).append(e.markup),g+=r.ajax_item_load,k+=r?.ajax_item_load,p=!1,"yes"===o.loadmore_btn&&d.html("Load More"),n(e.markup).length<r.ajax_item_load&&(c=!0,d.hide(),l.hide()),"yes"===t){function f(){i||(i=window.setInterval((function(){a.length?(n(a.shift()).addClass("is-inview"),f()):(window.clearInterval(i),i=null)}),u))}!function(i,n){var t=arguments.length>2&&void 0!==arguments[2]?arguments[2]:{};t.rootMargin=t.rootMargin||"10% 0px 0px 0px",new IntersectionObserver((function(i,e){i.forEach((function(i){i.isIntersecting&&(n(i),t.loop||e.unobserve(i.target))}))}),t).observe(i)}(n(".upk-ajax-grid .upk-item:not(.is-inview)")[0],(function(){a.push(n(".upk-ajax-grid .upk-item:not(.is-inview)")),f()}),{root:null,rootMargin:"0px",threshold:.8})}}})};"yes"===o.loadmore_btn&&n(d).on("click",(function(){p?p=!1:(p=!0,d.html("loading..."),f())})),"yes"===o.infinite_scroll&&n(window).on("scroll",(function(){n(window).scrollTop()==n(document).height()-n(window).height()&&(n(".upk-ajax-loading").css("display","block"),f())}))};jQuery(window).on("elementor/frontend/init",(function(){["upk-amox-grid","upk-kalon-grid","upk-alex-grid","upk-alice-grid","upk-alter-grid","upk-elite-grid","upk-hazel-grid","upk-gratis-grid","upk-maple-grid","upk-pixina-grid","upk-ramble-grid","upk-wixer-grid","upk-tiny-list","upk-scott-list","upk-harold-list"].forEach((i=>{elementorFrontend.hooks.addAction(`frontend/element_ready/${i}.default`,t)}))}))}(jQuery,window.elementorFrontend);
